...
open class CDCrud {
    ...
    public func createNewObjectWithProperties(entityName: String, properties: [AnyHashable : Any]) -> NSManagedObject? {
        ...
        let maxId = readEntityMaxId(entityName: entityName)
        let objectId = maxId!+1
    ...
    public func readObjectWhereKey(entityName: String, predicate: String? = nil) -> NSManagedObject? {
        let context = persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
        fetchRequest.returnsObjectsAsFaults = false
    ...
    public func readAllObjects(entityName: String, predicate: String? = nil) -> [NSManagedObject]? {
        let context = persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
        fetchRequest.predicate = NSPredicate(format: predicate!)
        fetchRequest.returnsObjectsAsFaults = false
    ...
    public func readLastObject(entityName: String) -> NSManagedObject? {
        let context = persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
        fetchRequest.returnsObjectsAsFaults = false
    ...
    public func readEntityMaxId(entityName: String) -> Int64? {
        let context = persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
        fetchRequest.returnsObjectsAsFaults = false
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: "createdAt", ascending: false)]
        ...
        do {
            let objects = try context.fetch(fetchRequest)
            if (objects.count > 0) {
                let objct = objects.first as? NSManagedObject
                return objct?.value(forKey: "id") as! Int64
            }
            return 0 as? Int64
